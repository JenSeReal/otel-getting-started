...
class ChaosClient(UserInterface):
    def __init__(self, client: UserInterface, base_delay: int = 50):
        ...
        self.request_probability = (0.80, 0.15, 0.05)
        ...
        self.response_code_probability = (0.90, 0.10)

    def get_user(self, id: int) -> tuple[User, int]:
        faults_enabled = os.getenv("CHAOS", "false").lower() == "true"
        if faults_enabled:
            # add latency
            additional_latency = self.request_latency[
                random.choices(self.request_type, self.request_probability)[0]
            ]
            ...
            status = random.choices(
                self.response_status, self.response_code_probability
            )[0]
            if status == "success":
